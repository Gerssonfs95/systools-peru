...
export function ToolCard({ item }: { item: Tool }) {
  return <article className="card flex flex-col p-5"><div className="mb-5 grid size-12 place-items-center rounded-xl bg-[#16a8ff]/10 text-[#62d6ff]"><ToolIcon name={item.icon}/></div><span className="text-xs font-black uppercase tracking-wider text-[#62d6ff]">{item.category}</span><h3 className="mt-2 text-xl font-black">{item.name}</h3><p className="mt-2 min-h-12 flex-1 text-sm leading-6 text-[#93a1b6]">{item.description}</p><Link href={`/herramientas/${item.slug}`} className="btn-primary mt-5 text-sm">Usar herramienta <ArrowUpRight size={16}/></Link></article>;
}
