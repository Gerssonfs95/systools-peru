...
import { availableToolIcons } from "./tool-icon";
...
export function AdminDashboard(){
  ...
  return <section className="container-page py-12"><datalist id="tool-icons">{availableToolIcons.map(icon=><option key={icon} value={icon}/>)}</datalist><div className="mb-8 flex flex-wrap items-center justify-between gap-4"><div><span className="text-xs font-black uppercase tracking-[.2em] text-[#62d6ff]">Administración</span><h1 className="mt-2 text-4xl font-black">Gestión de contenido</h1></div><button onClick={logout} className="btn-secondary"><LogOut size={17}/> Salir</button></div>
